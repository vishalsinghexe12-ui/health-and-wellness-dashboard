...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($offer_id) {
        ...
        if ($update_image) {
            $query = "UPDATE offers_discounts SET title='$title', description='$description', discount_percentage='$discount', image_path='$image_path', valid_until='$valid_until', status='$status' WHERE id='$offer_id'";
        } else {
            $query = "UPDATE offers_discounts SET title='$title', description='$description', discount_percentage='$discount', valid_until='$valid_until', status='$status' WHERE id='$offer_id'";
        }
        $success_msg = "Offer updated successfully!";
    } else {
        // Insert new offer
        $query = "INSERT INTO offers_discounts (title, description, discount_percentage, image_path, valid_until, status) VALUES ('$title', '$description', '$discount', '$image_path', '$valid_until', '$status')";
